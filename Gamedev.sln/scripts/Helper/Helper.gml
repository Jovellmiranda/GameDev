...
function UnitAttack(){
	...
	if (unit.attackWillHit){
		for (var i = 0; i <ds_list_size(global.selectedTargets); i++){ //bug fix
		with(global.selectedTargets[|i]){ //bug fix
			if (defending){
				defending = false;
			}
    ...
    if (global.selectedUnit.attackWillHit && !cManager.skillSent){
        ...
        for(var i = 0; i < ds_list_size(global.selectedTargets); i++){
            with (global.selectedTargets[|i]){
                ...
                incomingDamage = global.selectedUnit.selectedSkill.healthChange; // Fixed typo in 'selectedUnit'
                state = HURT;
                layer_sequence_headpos(unitSequence, hurtStart);
            }
        }
